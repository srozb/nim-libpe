...
const
  ...
  IMAGE_FILE_RELOCS_STRIPPED* = (0x00000001).ImageCharacteristics ## ```
                                                                  ...
                                                                  ## ```
  IMAGE_FILE_EXECUTABLE_IMAGE* = (0x00000002).ImageCharacteristics ## ```
                                                                   ##   Image only. Indicates that the image file is valid and can be run.
                                                                   ##      If this flag is not set, it indicates a linker error.
                                                                   ## ```
  IMAGE_FILE_LINE_NUMS_STRIPPED* = (0x00000004).ImageCharacteristics ## ```
                                                                     ##   COFF line numbers have been removed.
                                                                     ##      Deprecated and should be zero.
                                                                     ## ```
  IMAGE_FILE_LOCAL_SYMS_STRIPPED* = (0x00000008).ImageCharacteristics ## ```
                                                                      ##   COFF symbol table entries for local symbols have been removed.
                                                                      ##      Deprecated and should be zero.
                                                                      ## ```
  IMAGE_FILE_AGGRESSIVE_WS_TRIM* = (0x00000010).ImageCharacteristics ## ```
                                                                     ##   Obsolete. Aggressively trim working set.
                                                                     ##      Deprecated in Windows 2000 and later. Must be zero.
                                                                     ## ```
  IMAGE_FILE_LARGE_ADDRESS_AWARE* = (0x00000020).ImageCharacteristics ## ```
                                                                      ##   App can handle > 2gb addresses.
                                                                      ## ```
  IMAGE_FILE_RESERVED* = (0x00000040).ImageCharacteristics ## ```
                                                           ##   Reserved for future use.
                                                           ## ```
  IMAGE_FILE_BYTES_REVERSED_LO* = (0x00000080).ImageCharacteristics ## ```
                                                                    ##   Little endian: LSB precedes MSB in memory.
                                                                    ##      Deprecated and should be zero.
                                                                    ## ```
  IMAGE_FILE_32BIT_MACHINE* = (0x00000100).ImageCharacteristics ## ```
                                                                ##   Machine based on 32-bit-word architecture.
                                                                ## ```
  IMAGE_FILE_DEBUG_STRIPPED* = (0x00000200).ImageCharacteristics ## ```
                                                                 ##   Debugging information removed from image file.
                                                                 ## ```
  IMAGE_FILE_REMOVABLE_RUN_FROM_SWAP* = (0x00000400).ImageCharacteristics ## ```
                                                                          ##   If image is on removable media, fully load it and copy it to the
                                                                          ##      swap file.
                                                                          ## ```
  IMAGE_FILE_NET_RUN_FROM_SWAP* = (0x00000800).ImageCharacteristics ## ```
                                                                    ##   If image is on network media, fully load it and copy it to the
                                                                    ##      swap file.
                                                                    ## ```
  IMAGE_FILE_SYSTEM* = (0x00001000).ImageCharacteristics ## ```
                                                         ##   The image file is a system file, not a user program.
                                                         ## ```
  IMAGE_FILE_DLL* = (0x00002000).ImageCharacteristics ## ```
                                                      ...
                                                      ## ```
  IMAGE_FILE_UP_SYSTEM_ONLY* = (0x00004000).ImageCharacteristics ## ```
                                                                 ##   File should be run only on a UP machine.
                                                                 ## ```
  IMAGE_FILE_BYTES_REVERSED_HI* = (0x00008000).ImageCharacteristics ## ```
